...
def run_ga(
    ...
    with open(metrics_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["gen", "best_fitness", "mean_fitness", "std_fitness", "mean_links", "max_links"]
        )
        ...
        for gen in range(generations):
            ...
            if best_fit > best_so_far + 1e-9:
                ...
                genome.Genome.to_csv(
                    elite_src.dna, str(elites_dir / f"elite_gen_{gen:04d}.csv")
                )
            ...
            for _ in range(pop_size):
                ...
                # Motor-control-only evolution
                dna = genome.Genome.point_mutate(
                    dna, rate=point_mutate_rate, amount=point_mutate_amount
                )

                # Morphology evolution DISABLED
                # dna = genome.Genome.shrink_mutate(dna, rate=...)
                # dna = genome.Genome.grow_mutate(dna, rate=...)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run", default="run_06_motor_only")
